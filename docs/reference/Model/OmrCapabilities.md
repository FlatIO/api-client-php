...
------------ | ------------- | ------------- | -------------
**steps** | [**\Flat\APIClient\Model\OmrStepName[]**](OmrStepName.md) | Interactive steps this server supports. |
**formats** | **string[]** | Export formats available via &#x60;getOmrJobExport&#x60;. |
**outputs** | [**\Flat\APIClient\Model\OmrJobOutput[]**](OmrJobOutput.md) | Output destinations the account can use when creating a job. |
...
**locales** | **string[]** | Locales selectable for OCR, as BCP 47 codes sorted alphabetically.  These are the locales the recognition pipeline can actually read, which is neither the list of Flat interface locales nor a fixed set: new languages are added over time. Clients should default to the user&#39;s own locale when it appears here. |
**locales_details** | [**\Flat\APIClient\Model\OmrLocaleDetails[]**](OmrLocaleDetails.md) | The same locales as &#x60;locales&#x60;, each with its English display name, sorted alphabetically by &#x60;name&#x60; and ready to bind to a language picker.  Prefer this over &#x60;locales&#x60; when rendering a selector: it saves clients from shipping their own code-to-label table. |
