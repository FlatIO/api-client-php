#!/usr/bin/env python3
"""Emit OPERATIONS.json for the php-nextgen SDK (FR-001, FR-002).
...
def main() -> None:
    ...
    for path in (ROOT / "src" / "Api").glob("*.php"):
        for match in re.finditer(r"public function (\w+)\(", path.read_text()):
            symbols.setdefault(match.group(1), path.stem)

    models = set()
    for path in (ROOT / "src" / "Model").glob("*.php"):
        text = path.read_text()
        models |= set(re.findall(r"^(?:class|enum|interface) (\w+)\b", text, re.M))
    ...
    inventory = {
        ...
        "generator": {"name": "php-nextgen", "version": "7.24.0"},
